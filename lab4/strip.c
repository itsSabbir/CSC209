...
// Function to remove trailing question marks from a string
int strip_q_marks(char* inputStr) {
    // Initialize counter for number of question marks removed
    int count = 0;
    ...
    while (length > 0 && inputStr[length - 1] == '?') {
        inputStr[length - 1] = '\0';  // Null-terminate to remove the question mark
        count++;  // Increment the counter
        length--;  // Move to the previous character
    }

    // Return the number of question marks removed
    return count;
}
