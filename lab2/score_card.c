...
int sum_card(int **score_card, int size) {
    ...
    // This will be used to accumulate the sum of all the integers.
    int total = 0;

    // Loop through each pointer in the score_card array.
    // The loop starts from the first pointer (index 0) and goes up to the last pointer (index size-1).
    for (int i = 0; i < size; i++) {
        // Dereference the current pointer to get the integer it's pointing to.
        // Add that integer to the running total.
        total += *score_card[i];
    }

    // After summing up all the integers, return the final total.
    return total;
}
