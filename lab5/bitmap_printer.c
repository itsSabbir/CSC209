...
int main(int argc, char **argv) {
    ...
    if (pixel_array_offset <= 0 || width <= 0 || height <= 0) {
        fprintf(stderr, "Invalid metadata values in the bitmap file.\n");
        fclose(image);
        exit(1);
    }
    ...
    // Free the dynamically allocated memory for the pixel data
