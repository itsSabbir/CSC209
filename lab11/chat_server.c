...
int read_from(int client_index, struct sockname *users) {
    ...
    if (users[client_index].username == NULL) {
        ...
        users[client_index].username = strdup(buf);
    } else {
        // Add the username prefix and send the message
        char message[BUF_SIZE + strlen(users[client_index].username) + 3];
        sprintf(message, "%s: %s", users[client_index].username, buf);

        // Broadcast the message to all connected clients
        for (int i = 0; i < MAX_CONNECTIONS; i++) {
            if (users[i].sock_fd != -1 && i != client_index) {
                if (write(users[i].sock_fd, message, strlen(message)) != strlen(message)) {
                    users[i].sock_fd = -1;
                }
            }
        }
